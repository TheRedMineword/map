...
window.MapGenOnClick = function (obj) {
  ...
  try {
    // Case 1: embed is true
    if (obj.onclick.embed === true) {
      console.log("[MapGen] embed true");

      // If obj.onclick.data exists and has content, use it
      const embedData = (obj.onclick.data && Object.keys(obj.onclick.data).length > 0)
        ? obj.onclick.data
        : (() => {
            // Generate default embed if empty
            const corpMapRaw = window.vars?.CORPORATION_MAP_IDS || [];
            let corpMap = Array.isArray(corpMapRaw) ? corpMapRaw : 
                          (typeof corpMapRaw === 'string' ? JSON.parse(corpMapRaw) : []);

            const findFullId = slug => {
              const entry = corpMap.find(e => e.id === slug);
              return entry?.hsid || slug;
            };

            const c1Full = findFullId(obj.c1 || obj.onclick.c1);
            const c2Full = findFullId(obj.c2 || obj.onclick.c2);

            return {
              color: "#00b0f4",
              title: `Match: ${obj.onclick.match || 'Unknown'}`,
              description: `Corporation 1: **${c1Full}**\nCorporation 2: **${c2Full}**`,
              fields: [
                { name: "C1 Slug", value: obj.c1 || obj.onclick.c1 || 'N/A' },
                { name: "C2 Slug", value: obj.c2 || obj.onclick.c2 || 'N/A' }
              ],
              footer: { text: "Match Viewer" },
              timestamp: new Date().toISOString()
            };
          })();

      // Use your embed renderer
      if (window.createDiscordEmbed) {
        window.createDiscordEmbed(embedData, { x: 50, y: 50 });
      } else if (window.createEmbed) {
        window.createEmbed(embedData);
      } else {
        console.warn("[MapGen] no embed renderer found");
      }

    } 
    // Case 2: embed is false and data is string → execute as code
    else if (obj.onclick.embed === false && typeof obj.onclick.data === "string") {
      console.log("[MapGen] executing code string");
      new Function(obj.onclick.data)();
    } 
...
content.style.paddingRight = '6px'; // optional, space for scrollbar
  ...
  if (data.fields && Array.isArray(data.fields)) {
    data.fields.forEach(field => {
      ...
      fieldDiv.innerHTML = `<strong>${field.name}</strong>:<br>${parseMarkdown(field.value)}`;
...
window.onMatchClick = function(matchObj) {
  console.log("[MapGen] onMatchClick triggered", matchObj);

  if (!matchObj || !matchObj.c1 || !matchObj.c2) {
    console.warn("[MapGen] missing c1 or c2", matchObj);
    return;
  }

  let corpMap = window.vars?.CORPORATION_MAP_IDS;

  // Ensure corpMap is an array
  if (typeof corpMap === 'string') {
    try {
      corpMap = JSON.parse(corpMap);
    } catch (err) {
      console.warn("[MapGen] failed to parse CORPORATION_MAP_IDS", err);
      corpMap = [];
    }
  }
  if (!Array.isArray(corpMap)) corpMap = [];

  const findFullId = (slug) => {
    const entry = corpMap.find(e => e.id === slug);
    return entry?.hsid || slug; // fallback if not found
  };

  const fullC1 = findFullId(matchObj.c1);
  const fullC2 = findFullId(matchObj.c2);

  console.log("[MapGen] Resolved c1/c2:", fullC1, fullC2);

  // If empty embed data, generate a default one
  const embedData = matchObj.data && Object.keys(matchObj.data).length > 0 
    ? matchObj.data
    : {
        color: "#00b0f4",
        title: `Match: ${matchObj.match}`,
        description: `Corporation 1: **${fullC1}**\nCorporation 2: **${fullC2}**`,
        fields: [
          { name: "C1 Slug", value: matchObj.c1 },
          { name: "C2 Slug", value: matchObj.c2 }
        ],
        footer: { text: "Match Viewer" },
        timestamp: new Date().toISOString()
      };
  ...
  if (window.createDiscordEmbed) {
    ...
    console.warn("[MapGen] createDiscordEmbed function not found");
  }
};
